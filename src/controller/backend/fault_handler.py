...
class FaultHandler:
    ...
    def handle_port_down(self, dpid: int, port: int) -> None:
        ...
        LOG.warning("FaultHandler: PORT DOWN dpid=%s port=%d — starting recovery",
                     hex(dpid), port)

        link = self.topo_mgr.resolve_link(dpid, port)
        if link:
            LOG.info("FaultHandler: resolved to link %s:%d → %s:%d",
                     hex(link.src_dpid), link.src_port,
                     hex(link.dst_dpid), link.dst_port)
        else:
            LOG.info("FaultHandler: port %s:%d is an edge port (no link to resolve)",
                     hex(dpid), port)
    ...
    def handle_link_down(self, link: LinkKey) -> None:
        ...
        LOG.warning("FaultHandler: LINK DOWN %s:%d → %s:%d — starting recovery",
                     hex(link.src_dpid), link.src_port,
                     hex(link.dst_dpid), link.dst_port)
        ...
        LOG.info("FaultHandler: recovery complete for link %s:%d → %s:%d",
                 hex(link.src_dpid), link.src_port,
                 hex(link.dst_dpid), link.dst_port)
    ...
    def _refresh_flood_topology(self) -> None:
        ...
        LOG.info("FaultHandler: flood topology refreshed for %d switches",
